...
try:
    from .debug_utils import debug_log, format_layer_names, format_tensor_info
...
class ExrProcessor:
    ...
    def scan_exr_metadata(image_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not input_file:
                raise OSError(f"Could not open {image_path}")
            ...
            debug_log(logger, "error", "Error scanning EXR metadata", f"Error scanning EXR metadata from {image_path}: {e!s}")
            raise
    ...
    def process_default_channels(all_data, channel_names, height, width, normalize):
        ...
        if 'R' in channel_names and 'G' in channel_names and 'B' in channel_names:
            ...
            rgb_array = all_data[:, :, :3] if all_data.shape[2] >= 3 else np.stack([all_data[:, :, 0]] * 3, axis=2)
    ...
    def process_rgb_type_layer(group_name, r_suffix, g_suffix, b_suffix, a_suffix,
        ...
        try:
            ...
            debug_log(logger, "warning", "Error processing RGB layer", f"Error processing RGB layer {group_name}: {e!s}")
    ...
    def process_xyz_type_layer(group_name, x_suffix, y_suffix, z_suffix,
        ...
        try:
            ...
            debug_log(logger, "warning", "Error processing XYZ layer", f"Error processing XYZ layer {group_name}: {e!s}")
    ...
    def create_processed_layer_names(layers_dict, cryptomatte_dict):
        ...
        for crypto_name in cryptomatte_dict:
                ...
                if len(channel) == 1 and channel in 'RGBAXYZ':
                    if all(c in channel_names for c in 'RGB'):
                        if 'RGB' not in groups:
                            groups['RGB'] = []
                    elif all(c in channel_names for c in 'XYZ') and 'XYZ' not in groups:
                        groups['XYZ'] = []
    ...
    def process_exr_data(image_path: str, normalize: bool, node_id: str = None, layer_data: dict = None) -> list:
        ...
        try:
            ...
            debug_log(logger, "error", "Error loading EXR", f"Error loading EXR file {image_path}: {e!s}")
            raise
